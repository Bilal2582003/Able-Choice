...
if (isset($_POST['action'])) {
    ...
    if (isset($_POST['forTable'])) {
        ...
        if ($_POST['action'] === 'active') {
            ...
            $sql = "SELECT order_items.*,product.image1 as image, product.name as pName, orders.payment_method as mode,orders.id as orderId FROM order_items 
    ...
    if ($_POST['action'] == 'parameterOrderId' && !isset($_POST['forTable'])) {
        $id = $_POST['search'];
        $output = '';
        $query = "SELECT order_items.*,product.image1 as image, product.name as pName, orders.payment_method as mode,orders.id as orderId FROM order_items 
            JOIN orders ON order_items.order_id = orders.id 
            JOIN product ON order_items.product_id = product.id where orders.id='$id' and deleted_at is null
            ";
        $res = mysqli_query($con, $query);
        if (mysqli_num_rows($res) > 0) {
            while ($row = mysqli_fetch_assoc($res)) {
    ...
    if ($_POST['action'] == 'search' && !isset($_POST['forTable'])) {
        ...
        $query = "SELECT order_items.*,product.image1 as image, product.name as pName, orders.payment_method as mode,orders.id as orderId FROM order_items 
            JOIN orders ON order_items.order_id = orders.id 
            JOIN product ON order_items.product_id = product.id where (orders.id='$id' or product.name like '%$id%' or orders.payment_method like '%$id%' ) and orders.ip_address = '$token' and deleted_at is null
            ";
        $res = mysqli_query($con, $query);
        if (mysqli_num_rows($res) > 0) {
            while ($row = mysqli_fetch_assoc($res)) {
